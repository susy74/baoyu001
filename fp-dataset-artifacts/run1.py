...
def main():
    ...
    if training_args.do_train:
        ...
        train_dataset = dataset[eval_split]
    ...
    class LogitsAndLabelsSaverCallback(TrainerCallback):
        ...
        def on_evaluate(self, args, state, control, **kwargs):
            ...
            # Convert logits to numpy array and then to list for JSON serialization
                logits = logits.detach().cpu().numpy()
                labels = [self.train_dataset[i]['label'] for i in range(len(self.train_dataset))]

                # Save logits, labels, and their IDs for the current epoch in .jsonl format
                correct = 0
                with open(f'logits_and_labels_epoch_{state.epoch}.jsonl', 'w') as f:
                    for i, (logit, label) in enumerate(zip(logits, labels)):
                        p_label = int(logit.argmax())
                        confidence = logit[label]
                        if p_label == label:
                            correct = 1
                                
                        # Convert each logit tensor to a list
                        logit_list = logit.tolist()
                        # Create a JSON record for each example
                        json_record = json.dumps({'id': i, 'logit': logit_list, 'correctness': correct, 'confidence': confidence})
                        # Write the record to the .jsonl file
                        f.write(json_record + '\n')
    callback = LogitsAndLabelsSaverCallback(train_dataset)
